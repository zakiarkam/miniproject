...
async function uploadFileToS3(file: Buffer, fileName: string): Promise<string> {
  ...
  const objectUrl = `https://${process.env.NEXT_PUBLIC_AWS_S3_BUCKET_NAME}.s3.${process.env.NEXT_PUBLIC_AWS_S3_REGION}.amazonaws.com/pagebuilder/${fileName}`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    // XSS Prevention - R4 Policy: Sanitize HTML from page builder
    // PageBuilder content must not contain scripts or dangerous content
    ...
    if (validation.hasScripts || validation.hasDangerousContent) {
      ...
      return NextResponse.json(
        {
          error: "Security violation: Page builder HTML contains prohibited scripts or dangerous content",
          details: {
    ...
    logXSSPreventionEvent({
      action: "PAGEBUILDER_UPLOAD_SANITIZED",
      blocked: false,
      endpoint: "/api/v1/aws/s3-upload/pagebuilder",
      ipAddress: getClientIP(request.headers),
      userAgent: getUserAgent(request.headers),
